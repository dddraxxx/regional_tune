...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_args, tokenizer: transformers.PreTrainedTokenizer):
        ...
        # Process medical format
        processed_data = []
        for item in list_data_dict:
            # Parse question and options from input field
            question, options_dict = self._parse_input(item['input'])

            # Format instruction with question + options
            instruction = self._format_instruction(question, options_dict)
        ...
        self.list_data_dict = processed_data
    ...
    def _parse_input(self, input_text):
        """Extract question and options from input text"""
        # Split into question and options parts
        q_part = input_text.split('Q:')[1]
        question = q_part.split('{')[0].strip()
        options_str = q_part.split('{')[1].split('}')[0]
        return question, eval('{' + options_str + '}')

    def _format_instruction(self, question, options):
        """Format question with options in consistent format"""
        formatted = f"{question}\n\nChoices:\n"
        for opt, text in sorted(options.items()):  # Sort to ensure A,B,C,D order
            formatted += f"{opt}. {text}\n"
...
def print_dataset_examples(dataset, num_examples=2, log_file="data_sample.log"):
    """Print example data from the dataset to a log file.
    ...
    """
    with open(log_file, 'w') as f:
        f.write(f"{'='*40} Dataset Examples {'='*40}\n")
        for i in range(min(num_examples, len(dataset.list_data_dict))):
            ...
            f.write(f"Input: {example.get('input', '')}\n")
            f.write(f"Output: {example['output']}\n")
            f.write(f"\nFormatted Source:\n")
            f.write(f"{dataset.sources[i]}\n")
            f.write(f"Target: {dataset.targets[i]}\n")
            f.write(f"{'-'*90}\n")

    logging.info(f"Dataset examples logged to {log_file}")
def train():
    ...
    if training_args.local_rank in {-1, 0}:
        ...
        print_dataset_examples(data_module['train_dataset'], 2, os.path.join(training_args.output_dir, "data_sample.log"))
