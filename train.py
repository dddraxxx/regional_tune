...
# Updated prompt handling with dataset type awareness
PROMPT_TEMPLATES = {
    "math": {
        "input": (
            "Solve this math problem. Include your reasoning and end with 'The answer is: [answer]'.\n\n"
            "### Instruction:\n{instruction}\n\n### Input:\n{input}\n\n### Response:"
        ),
        "no_input": (
            "Solve this math problem. Include your reasoning and end with 'The answer is: [answer]'.\n\n"
    ...
    "code": {
        "input": (
            "Write Python code to solve this task. Follow the input specifications exactly.\n\n"
            "### Instruction:\n{instruction}\n\n### Input:\n{input}\n\n### Response:\n"
        ),
        "no_input": (
...
class DataArguments:
    ...
    dataset_type: str = field(
        default="code",  # New argument to specify dataset type
        metadata={"help": "Dataset type: 'code' or 'math'"}
    )
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    code_context_length: int = field(
        default=2048,
        metadata={"help": "Max sequence length for code examples"}
    )

    def __post_init__(self):
        # Set context length based on dataset type
        if self.dataset_type == "code":
            self.model_max_length = self.code_context_length
        super().__post_init__()
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_args, tokenizer: transformers.PreTrainedTokenizer):
        ...
        self.prompt_input = PROMPT_TEMPLATES[dataset_type]["input"]
        self.prompt_no_input = PROMPT_TEMPLATES[dataset_type]["no_input"]
        ...
        if dataset_type == "code":
            self.list_data_dict = [{
                ...
                'input': ex.get('input', ''),
                'output': self._format_code_output(ex['output'])
            } for ex in self.list_data_dict]
        elif dataset_type == "math":
            self.list_data_dict = [{
                'instruction': item['question'],
                'input': '',
                'output': f"{item['answer'].split('####')[0].strip()} The answer is: {item['answer'].split('####')[-1].strip()}"
            } for item in self.list_data_dict]

        sources = [
            self.prompt_input.format_map(example) if example.get("input", "") != "" else self.prompt_no_input.format_map(example)
